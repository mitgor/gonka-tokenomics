"""UAE fleet on Gonka, v2: recalibrated to Gonka founder feedback (David Liberman, 2026-09-19).

Changes vs v1: MI300X weight 476 (founder-implied 1.6x on-demand yield -> 1.11x H100, 0.49x H200),
MI300X throughput 1,100 tok/s, MI300X on-demand $1.80; B300 reference (2.1x on-demand, weight ~2,724);
long-term contract rate 0.4-0.5x on-demand as the market baseline; scenarios re-anchored to the
founders' price path ($2 equilibrium after a 10k-MI300X ramp; $3 / $4.50 / $14 at 10/20/50% MoM growth).
...
FLEET = {
    ...
    "MI300X": dict(n=10_000, weight=476,      tps=1_100,  kw=1.10, rent=1.80, capex=18_000,    live=False),
...
SCENARIOS = {
    ...
    "B. Post-ramp equilibrium":        2.00,
    "C. 20% MoM organic growth":       4.50,
...
REV_SCENARIOS = {
    "Medium":     dict(gnk=2.00, fill=0.55, usd_per_m_out=1.00, rent_util=0.60, rent_mult=1.00),
    "Optimistic": dict(gnk=4.50, fill=0.80, usd_per_m_out=2.00, rent_util=0.80, rent_mult=1.13),
}
...
# ---- founder calibration (David Liberman, 2026-09-19) ----
FOUNDER = {  # yield multiple of on-demand $/hr at the current network state, and the low point seen
    "MI300X": dict(od=1.80, mult=1.6, low=0.9),
    "H200":   dict(od=4.50, mult=1.3, low=0.7),
    "B300":   dict(od=7.85, mult=2.1, low=1.2),
}
CONTRACT = (0.40, 0.50)                 # long-term contracts pay 0.4-0.5x on-demand
GROWTH = {0.10: (1.5, 3.0), 0.20: (2.3, 4.5), 0.50: (7.0, 14.0)}   # MoM growth -> (multiple, GNK $)


def eq_price(units, k):
    """GNK price at which a joined fleet's per-GPU mining yield settles at k x on-demand."""
    W = sum(n * w for n, w, _ in units)
    share = min(W / (W + EXISTING_WEIGHT), PARTICIPANT_CAP * N_PARTICIPANTS)
    return sum(n * od * 24 * k for n, _, od in units) / (emission_per_day() * share)


def founder_calibration():
    per_w = emission_per_day() / EXISTING_WEIGHT
    print("\n=== FOUNDER CALIBRATION (current network, 494 GPUs) ===")
    for name, f in FOUNDER.items():
        usd = f["od"] * 24 * f["mult"]; gnk = usd / GNK_SPOT; w = gnk / per_w
        print(f"  {name:6s} {f['mult']}x on-demand ${f['od']} = ${usd:6.1f}/day = {gnk:5.0f} GNK/day -> weight {w:5.0f} "
              f"({w/W_H100:.2f}x H100, {w/970:.2f}x H200); low {f['low']}x; vs contract {f['mult']/CONTRACT[1]:.1f}-{f['mult']/CONTRACT[0]:.1f}x")
    mi = (10_000, FLEET["MI300X"]["weight"], 1.80)
    print(f"  10k MI300X equilibrium: ${eq_price([mi],1.0):.2f} @1.0x on-demand, ${eq_price([mi],1.2):.2f} @1.2x, "
          f"${eq_price([mi],1.6):.2f} @1.6x (founder: ~$2)")
    both = [(20_000, 970, 4.50), mi]
    print(f"  20k H200 alone @1.0x: ${eq_price([both[0]],1.0):.2f}; both tranches @1.0x: ${eq_price(both,1.0):.2f}  (why H200 is not the story)")
    for g, (mult, px) in GROWTH.items():
        print(f"  organic growth {g:.0%} MoM for 6 months: founder {mult}x -> ${px:.2f} (straight compounding {(1+g)**6:.1f}x)")


def ladder_for(name, sizes, od):
    a = FLEET[name]
    print(f"\n=== TRANCHE LADDER: {name} (mining only, 90% uptime; on-demand ${od}/hr, contract ${od*CONTRACT[0]:.2f}-{od*CONTRACT[1]:.2f}) ===")
    print(f"{'units':>7} {'share':>6} {'GNK/unit/d':>10} {'$/d@spot':>9} {'x on-demand':>11} {'x contract':>10} {'eq GNK @1.2x':>12} {'collateral':>11}")
    for n in sizes:
        w = n * a["weight"]; share = min(w / (EXISTING_WEIGHT + w), PARTICIPANT_CAP * N_PARTICIPANTS)
        gnk = emission_per_day() * share / n * UPTIME; usd = gnk * GNK_SPOT
        col = w * (1 - BASE_WEIGHT) * COLLATERAL_GNK_PER_WEIGHT
        print(f"{n:>7,} {share:>6.0%} {gnk:>10.0f} {usd:>9.2f} {usd/(od*24):>11.2f} {usd/(od*24*CONTRACT[1]):>10.2f} "
              f"{eq_price([(n, a['weight'], od)],1.2):>12.2f} {col/1e6:>9.1f}M")


if __name__ == "__main__":
    founder_calibration()
    ladder_for("MI300X", (1_000, 2_000, 5_000, 10_000), 1.80)
    ladder_for("H200", (500, 2_000, 5_000, 20_000), 4.50)
